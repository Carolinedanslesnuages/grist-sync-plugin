...
describe('gristWidget', () => {
  ...
  describe('initializeGristWidget', () => {
    ...
    it('devrait détecter les paramètres de requête', async () => {
    ...
    it('devrait détecter un docId seul dans les paramètres de requête', async () => {
      ...
      expect(result.isInGrist).toBe(true);
      expect(result.docId).toBe('test-only-doc');
    });

    it('devrait détecter le referrer', async () => {
      // Clean window.grist to ensure we don't use widget API
      delete (window as any).grist;
      
      // Mock document.referrer
      Object.defineProperty(document, 'referrer', {
        value: 'http://localhost:8484/doc/referrer-doc-id/p/Table1',
        writable: true,
        configurable: true,
      });
    ...
    it('devrait combiner plusieurs stratégies de détection', async () => {
      // Clean window.grist
      delete (window as any).grist;
      
      // Mock referrer with docId
      Object.defineProperty(document, 'referrer', {
        value: 'http://localhost:8484/doc/referrer-doc',
        writable: true,
        configurable: true,
      });

      // Mock query params with apiToken (should override/add to referrer info)
      Object.defineProperty(window, 'location', {
        value: {
          origin: 'http://localhost:5173',
          pathname: '/',
      ...
      expect(result.isInGrist).toBe(true);
      expect(result.docId).toBe('referrer-doc');
      expect(result.gristApiUrl).toBe('http://localhost:8484');
